...
def combine_coco_captions(annotation_path):
    val_caption_path = os.path.join(annotation_path, 'captions_val2014.json')
    if not os.path.exists(val_caption_path):
        raise Exception("Missing captions_val2014.json in %s" % annotation_path)

    val_caps = json.load(open(val_caption_path))

    all_caps = {
        'info': val_caps['info'],
        'licenses': val_caps['licenses'],
...
def combine_coco_instances(annotation_path):
    if not os.path.exists('%s/instances_val2014.json' % annotation_path):
        raise Exception("Missing val2014 annotation.")

    val_instances = json.load(open('%s/instances_val2014.json' % annotation_path))
    all_instances = {
        'info': val_instances['info'],
        'licenses': val_instances['licenses'],
        'type': val_instances.get('type', 'instances'),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cap_file", type=str, default='./ours_captions/COCO+Ours+id.json',
                        help="path towards json or jsonl saving image ids and their captions in list of dict.")
    parser.add_argument("--image_id_key", type=str, default="image_id",         #########image_id
                        help="in each dict of cap_file, which key stores image id of coco.")
    parser.add_argument("--caption_key", type=str, default="texts_13_",         ##############对应于描述文本的关键字：detail_caption，final_texts_,refined_texts
    ...
    parser.add_argument("--coco_path", type=str, default='/home/MLLMs/CHAIR-COCO',
                        help="only use for regenerating CHAIR evaluator object, will be ignored if uses cached evaluator.")
    
    parser.add_argument("--save_path", type=str, default="./chair_results/llava_1.5_13b_ours_chair_results.json",
